...
export default function AppointmentForm(
  { onSubmit, initial, doctors, patients }: {
  ...
  useEffect(() => {
    setPId(initial?.patient_id || 0);
    setDId(initial?.doctor_id || 0);
    if (initial) {
      setDate(
        initial.time.slice(8, 10) + "-" + initial.time.slice(5, 7) + "-" +
          initial.time.slice(0, 4),
      );
      setTime(initial.time.slice(11, 16));
    ...
    <form onSubmit={submit} className="space-y-2 bg-white p-4 rounded shadow">
      <div className="flex gap-2">
        ...
        >
          ...
          {patients.map((p) => <option key={p.id} value={p.id}>{p.name}
          </option>)}
        </select>
        <select
          value={dId}
        ...
        >
          ...
          {doctors.map((d) => <option key={d.id} value={d.id}>{d.name}
          </option>)}
        </select>
      </div>
      <div className="flex gap-2">
